...
#define CY_FX_BULKSRCSINK_DMA_BUF_COUNT      (2)                       /* Bulk channel buffer count */
...
/*
 * There are 8 EP-IN and 8 EP-OUT, total 16 endpoints.
 * Following define just indicates the base endpoint.
 * Offset must be added for each endpoints.
 */
#define CY_FX_EP_PRODUCER(index)        (0x08+(index))      /* EP 8+i OUT */
#define CY_FX_EP_CONSUMER(index)        (0x81+(index))      /* EP 1+i IN */

#define CY_FX_EP_PRODUCER_SOCKET(index)   (CY_U3P_UIB_SOCKET_PROD_8+(index))    /* Socket 8+i is producer base */
#define CY_FX_EP_CONSUMER_SOCKET(index)   (CY_U3P_UIB_SOCKET_CONS_1+(index))    /* Socket 1+i is consumer base */

#define CY_FX_N_SINK            (8)         /* EP_PRODUCER-CPU_CONSUMER */
#define CY_FX_N_SRC             (8)         /* CPU_PRODUCER-EP_CONSUMER */
