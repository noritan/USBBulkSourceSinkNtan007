...
CyU3PDmaChannel glChHandleBulkSrc[CY_FX_N_SRC];     /* DMA MANUAL_OUT channel handle.         */
...
/* Create a SINK pipe from EP-producer to CPU-consumer */
void
createSinkPipe(const uint16_t index, CyU3PUSBSpeed_t usbSpeed)
{
    uint16_t size = 0;
    ...
    apiRetStatus = CyU3PSetEpConfig(CY_FX_EP_PRODUCER(index), &epCfg);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
    ...
    CyU3PUsbFlushEp(CY_FX_EP_PRODUCER(index));
    ...
    dmaCfg.prodSckId = CY_FX_EP_PRODUCER_SOCKET(index);
    ...
    apiRetStatus = CyU3PDmaChannelCreate (&glChHandleBulkSink[index],
    ...
    apiRetStatus = CyU3PDmaChannelSetXfer (&glChHandleBulkSink[index], CY_FX_BULKSRCSINK_DMA_TX_SIZE);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
...
/* Create a SRC pipe from CPU-producer to EP-consumer */
void
createSrcPipe(const uint16_t index, CyU3PUSBSpeed_t usbSpeed)
{
    uint16_t size = 0;
    uint16_t i = 0;
    CyU3PEpConfig_t epCfg;
    ...
    apiRetStatus = CyU3PSetEpConfig(CY_FX_EP_CONSUMER(index), &epCfg);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
        CyU3PDebugPrint (4, "CyU3PSetEpConfig failed, Error code = %d\n", apiRetStatus);
        CyFxAppErrorHandler (apiRetStatus);
    ...
    /* Flush the endpoint memory */
    CyU3PUsbFlushEp(CY_FX_EP_CONSUMER(index));
    ...
    dmaCfg.consSckId = CY_FX_EP_CONSUMER_SOCKET(index);
    dmaCfg.dmaMode = CY_U3P_DMA_MODE_BYTE;
    ...
    apiRetStatus = CyU3PDmaChannelCreate (&glChHandleBulkSrc[index],
    ...
    apiRetStatus = CyU3PDmaChannelSetXfer (&glChHandleBulkSrc[index], CY_FX_BULKSRCSINK_DMA_TX_SIZE);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
        CyU3PDebugPrint (4, "CyU3PDmaChannelSetXfer failed, Error code = %d\n", apiRetStatus);
        CyFxAppErrorHandler(apiRetStatus);
    ...
    for (i = 0; i < CY_FX_BULKSRCSINK_DMA_BUF_COUNT; i++)
    {
        apiRetStatus = CyU3PDmaChannelGetBuffer (&glChHandleBulkSrc[index], &buf_p, CYU3P_NO_WAIT);
        if (apiRetStatus != CY_U3P_SUCCESS)
        {
            CyU3PDebugPrint (4, "CyU3PDmaChannelGetBuffer failed, Error code = %d\n", apiRetStatus);
            CyFxAppErrorHandler(apiRetStatus);
        ...
        apiRetStatus = CyU3PDmaChannelCommitBuffer (&glChHandleBulkSrc[index], buf_p.size, 0);
        if (apiRetStatus != CY_U3P_SUCCESS)
        {
            CyU3PDebugPrint (4, "CyU3PDmaChannelCommitBuffer failed, Error code = %d\n", apiRetStatus);
            CyFxAppErrorHandler(apiRetStatus);
...
/* Disable a SINK pipe from EP-producer to CPU-consumer */
void
disableSinkPipe(uint16_t index)
{
    CyU3PEpConfig_t epCfg;
    CyU3PReturnStatus_t apiRetStatus = CY_U3P_SUCCESS;

    /* Destroy the channels */
    CyU3PDmaChannelDestroy (&glChHandleBulkSink[index]);
...
/* Disable a SRC pipe from CPU-producer to EP-consumer */
void
disableSrcPipe(uint16_t index)
{
    CyU3PEpConfig_t epCfg;
    CyU3PReturnStatus_t apiRetStatus = CY_U3P_SUCCESS;

    /* Destroy the channels */
    CyU3PDmaChannelDestroy (&glChHandleBulkSrc[index]);
        ...
        void)
{
    ...
    for (i = 0; i < CY_FX_N_SINK; i++) {
        createSinkPipe(i, usbSpeed);
    }
    for (i = 0; i < CY_FX_N_SRC; i++) {
        createSrcPipe(i, usbSpeed);
    }
...
CyBool_t
haltSinkPipe(uint16_t index)
{
    uint16_t    ep_number = CY_FX_EP_PRODUCER(index);

    CyU3PDmaChannelReset (&glChHandleBulkSink[index]);
    CyU3PUsbFlushEp(ep_number);
    CyU3PUsbResetEp (ep_number);
    CyU3PDmaChannelSetXfer (&glChHandleBulkSink[index], CY_FX_BULKSRCSINK_DMA_TX_SIZE);
    CyU3PUsbStall (ep_number, CyFalse, CyTrue);
    CyU3PUsbAckSetup ();
    return CyTrue;
}

CyBool_t
haltSrcPipe(uint16_t index)
{
    uint16_t    ep_number = CY_FX_EP_CONSUMER(index);

    CyU3PDmaChannelReset (&glChHandleBulkSrc[index]);
    CyU3PUsbFlushEp(ep_number);
    CyU3PUsbResetEp (ep_number);
    CyU3PDmaChannelSetXfer (&glChHandleBulkSrc[index], CY_FX_BULKSRCSINK_DMA_TX_SIZE);
    CyU3PUsbStall (ep_number, CyFalse, CyTrue);
    CyU3PUsbAckSetup ();
    return CyTrue;
}
    ...
    )
{
    ...
    if (bType == CY_U3P_USB_STANDARD_RQT)
    {
                ...
                && (wValue == CY_U3P_USBX_FS_EP_HALT))
        {
            ...
            {
                if (
                    (wIndex >= CY_FX_EP_PRODUCER(0))
                    && (wIndex < CY_FX_EP_PRODUCER(CY_FX_N_SINK))
                )
                {
                    isHandled = haltSinkPipe(wIndex - CY_FX_EP_PRODUCER(0));
                }

                if (
                    (wIndex >= CY_FX_EP_CONSUMER(0))
                    && (wIndex < CY_FX_EP_CONSUMER(CY_FX_N_SRC))
                )
                {
                    isHandled = haltSrcPipe(wIndex - CY_FX_EP_CONSUMER(0));
                }
            }
        }
    }
